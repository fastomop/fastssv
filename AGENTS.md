...
2. Subclass `Rule` (`fastssv.core.base`) and decorate with `@register` (`fastssv.core.registry`). Set `rule_id = "<category>.<snake_name>"` (always flat per-category — the directory nesting under `domain_specific` doesn't appear in the id), plus `name`, `description`, `severity`, `suggested_fix`.
