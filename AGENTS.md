...
1. Create `src/fastssv/rules/<category>/<snake_name>.py` (no `rule_` prefix; match existing names like `datetime_between_date_literal.py`).
2. Subclass `Rule` (`fastssv.core.base`) and decorate with `@register` (`fastssv.core.registry`). Set `rule_id = "<category>.<snake_name>"`, plus `name`, `description`, `severity`, `suggested_fix`.
3. Add `from .<file> import <Class>` and append `"<Class>"` to `__all__` in `src/fastssv/rules/<category>/__init__.py`. (Leave `src/fastssv/rules/__init__.py` alone — it imports category packages, not individual rules.)
